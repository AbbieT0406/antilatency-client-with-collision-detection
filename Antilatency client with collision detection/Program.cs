...
internal class Program
{
    ...
    private static readonly Dictionary<ulong, Antilatency.Math.float3> _lastPositions = new(); //stores last known positions of each device
    private static readonly TimeSpan _collisionCooldown = TimeSpan.FromMilliseconds(500); //minimum time between collision alerts for the same device pair
    private static readonly Dictionary<(ulong, ulong), DateTime> _lastCollisionTimes = new(); //stores last collision alert times for each device pair
    private static float _collisionDistanceThreshold = 0.05f; //5cm threshold for collision detection THIS WILL NEED TO CHANGE BEFORE USING WITH THE ACTUAL ROBOTS!!!!
    ...
    private static void ProcessTrackingData(bool isBlue)
    {
        ...
        foreach (var altData in _altDevices.Values)
        {
            try
            {
                ...
                _lastPositions[altData.Id] = position;
        ...
        if ( _altDevices.Count >=2)
        {
            CollisionChecker(); //checks for collisions if 2 or more devices are connected
        }
    ...
    private static void PrintStatus() //self explanatory i wont explain this
    {
        ...
        Console.WriteLine($"Collision detection: ACTIVE. Threshold: {_collisionDistanceThreshold:F2}");
    ...
    private static void CollisionChecker()
    {

        var deviceIDs = _altDevices.Keys.ToList();

        MonitorDistances();

        for (int i =0; i< deviceIDs.Count; i++)
        {
            for(int j = i + 1; j< deviceIDs.Count; j++)
            {
                var id1 = deviceIDs[i];
                var id2 = deviceIDs[j];
                if (!_lastPositions.ContainsKey(id1) || !_lastPositions.ContainsKey(id2))
                    continue;

                var pos1 = _lastPositions[id1];
                var pos2 = _lastPositions[id2];

                float distance = CalculateDistance(pos1, pos2);

                if (distance <= _collisionDistanceThreshold)
                {
                    var devicePair = (Math.Min(id1, id2), Math.Max(id1, id2));
                    if (_lastCollisionTimes.ContainsKey(devicePair) &&
                        (DateTime.Now - _lastCollisionTimes[devicePair]) < _collisionCooldown)
                    {
                    ...
                    Console.WriteLine(" ");
                    Console.WriteLine(" ");
                    Console.WriteLine($"⚠️ COLLISION detected between devices {id1:X} and {id2:X} - Distance: {distance:F3}m");
                    Console.WriteLine(" ");
                    Console.WriteLine(" ");
                    Console.WriteLine(" ");
                    _lastCollisionTimes[devicePair] = DateTime.Now;
                }
            }
        }
        // Placeholder for collision detection logic
        // This function can be expanded to analyze positions of devices
        // and determine if any are within a certain threshold distance
        // indicating a potential collision.
    }

    private static float CalculateDistance(Antilatency.Math.float3 a, Antilatency.Math.float3 b)
    {
        float dx = a.x - b.x;
        float dy = a.y - b.y;
        float dz = a.z - b.z;
    ...
    private static void MonitorDistances()
    {
        var deviceIds = _altDevices.Keys.ToList();
        ...
        for (int i = 0; i < deviceIds.Count; i++)
        {
            for (int j = i + 1; j < deviceIds.Count; j++)
            {
                ...
                // Show real-time distances
                Console.WriteLine($"Devices {id1:X} ↔ {id2:X}: {distance:F3}m " +
                                $"(Threshold: {_collisionDistanceThreshold:F3}m) " +
                                $"(X: {Math.Abs(pos1.x - pos2.x):F3}, " +
                                $"Y: {Math.Abs(pos1.y - pos2.y):F3}, " +
                                $"Z: {Math.Abs(pos1.z - pos2.z):F3})");
            }
        }
    }
}